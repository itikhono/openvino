...
TSStridedSliceForward::TSStridedSliceForward() {
    ...
    auto sinking_transformation = [=](const std::shared_ptr<Node>& main_node,
        ...
        // align begin, end, stride inputs to insert Transpose for each of them
        ...
        for (size_t input_idx = 1; input_idx <= 3; ++input_idx) {
            ...
            // if input_rank less than the data input rank,
            // then we need to extend the input rank to the data input rank
            // to successfully execute Transpose operation
            if (num_elements < data_rank_val) {
                ...
                if (input_idx == 1) {
                    ...
                    input_const_val.resize(data_rank_val, 0);
                } else if (input_idx == 2) {
                    // 'end' input have to be initialized with the corresponding `data` input dim value
                    input_const_val.resize(data_rank_val);
                    for (size_t i = (data_rank_val - num_elements); i < data_rank_val; ++i) {
                    ...
                    input_const_val.resize(data_rank_val, 1);
                }
