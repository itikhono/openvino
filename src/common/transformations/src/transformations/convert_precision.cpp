    ...
    auto register_constants = [&const_to_internal_output](const std::vector<std::shared_ptr<Node>>& ops) {
        for (auto& node : ops) {
            for (auto& input : node->inputs()) {
                if (auto const_node =
                        std::dynamic_pointer_cast<ov::op::v0::Constant>(input.get_source_output().get_node_shared_ptr())) {
...
bool ov::pass::ConvertPrecision::run_on_model(const std::shared_ptr<ngraph::Function>& f) {
    ...
    type_to_fuse_map type_to_fuse{
        {ov::op::v0::Parameter::get_type_info_static(), fuse_type_to_parameter},
        ...
        {ov::op::v1::ReduceLogicalAnd::get_type_info_static(), fuse_type_to_reduce_logical<ov::op::v1::ReduceLogicalAnd>},
...
std::shared_ptr<ngraph::Node> change_constant_precision(std::shared_ptr<ov::op::v0::Constant>& constant) {
...
std::shared_ptr<Node> convert_low_precisions_int(std::shared_ptr<ov::op::v0::Constant>& constant, ov::element::Type to) {
