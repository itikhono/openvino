...
ov::pass::GRUCellFusion::GRUCellFusion() {
    ...
    matcher_pass_callback callback = [=](pattern::Matcher& m) {
        ...
        if (pattern_map.find(add_1) != pattern_map.end()) {
            ...
            bias_add_1 = rg.make<ov::op::v0::Constant>(WR.get_element_type(), Shape{1, static_cast<size_t>(2 * hidden_size)}, 0);
        }
        ...
        if (pattern_map.find(add_2) != pattern_map.end()) {
            ...
            bias_add_2 = rg.make<ov::op::v0::Constant>(WRh.get_element_type(), Shape{1, static_cast<size_t>(hidden_size)}, 0);
        }

        auto B = rg.make<ov::op::v0::Concat>(OutputVector{bias_add_1, bias_add_2}, 1);
